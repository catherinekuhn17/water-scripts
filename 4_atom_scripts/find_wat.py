...
def build_df(pdb_id, coord_list):
    ...
    ----------
        pdb_id : string
            pdb_id code of structure analyzed
        coord_list : list
            list of info that will be transformed into the dataframe
            
    Returns
    -------
        df : DataFrame
            the data frame with the water/protein info required for later analysis
    '''

    df = pd.DataFrame()
    df['id'] = np.array(coord_list, dtype=object)[:,0]
    df['chain'] = np.array(coord_list, dtype=object)[:,1]
    df['resi'] = np.array(coord_list, dtype=object)[:,2]
    df['resn'] = np.array(coord_list, dtype=object)[:,3]
    df['alt'] = np.array(coord_list, dtype=object)[:,4]
    df['resb'] = np.array(coord_list, dtype=object)[:,5]
    df['names'] = np.array(coord_list, dtype=object)[:,6]
    df['un_names'] = np.array(coord_list, dtype=object)[:,7]
    df['prot_coord'] = np.array(coord_list, dtype=object)[:,8]
    df['wat_coord'] = np.array(coord_list, dtype=object)[:,9]
    df['b'] = np.array(coord_list, dtype=object)[:,10]
    ...
    df['dih'] = dih
    return df
   # df.to_csv(f'/Users/catherinekuhn/Downloads/wataatest{pdb_id}.csv')
...
def build_dictionaries(df, out_dir):
    ...
    for at in poss_atom_combo:
        ...
        for label in np.unique(bucket): # for each dihedral cluser
            ...
            new_p_coord, new_wt_coord, norm_b_val, wat_q_val = obtain_new_xyz(df,
                                                                              max_point[label],
                                                                              indx, 
                                                                              at)
            dih_info_tmp[label] = (max_point[label], new_p_coord, new_wt_coord, norm_b_val, wat_q_val)
